...
class ClassificationNet():
	...
	def compile_theano_functions(self, data_type='2D', loss='cross_entropy'):
		...
		prediction_train_smax = get_output(self.net[self.cf.out_layer], train_args['X'], deterministic=False)
		prediction_val_smax = get_output(self.net[self.cf.out_layer], val_args['X'], deterministic=True)
		...
		one_hot_target_train = get_one_hot_class_target(train_args['y'], self.cf.num_classes)
		one_hot_target_val = get_one_hot_class_target(val_args['y'], self.cf.num_classes)

		### cross entropy loss
		if loss=='cross_entropy':
			self.loss['train'] = categorical_crossentropy(prediction_train_smax, one_hot_target_train).mean()
			self.loss['val'] = categorical_crossentropy(prediction_val_smax, one_hot_target_val).mean()
		...
		self.train_fn = theano.function(train_args.values(), [self.loss['train'], train_acc], updates=updates)
		self.val_fn = theano.function(val_args.values(), [self.loss['val'], val_acc, prediction_val_smax])
	...
	def run_epoch(self, batch_gen):
		...
		for _ in range(self.cf.num_val_batches):
			...
			args = (batch['data'], batch['class_target'])

			val_loss, val_acc, val_preds = self.val_fn(*args)
		...
		for _ in range(self.cf.num_train_batches):
			...
			args = (batch['data'], batch['class_target'], lr)

			train_loss, train_acc = self.train_fn(*args)
