...
import numpy as np
from DeepTrainerUtils.utils import get_one_hot_prediction, get_weights
# from DeepTrainerUtils.utils import binary_dice_per_batch_and_class as binary_dice_per_instance_and_class
from DeepTrainerUtils.utils import binary_dice_per_instance_and_class
...
class SegmentationNet():
	...
	def compile_theano_functions(self, data_type='2D', loss='cross_entropy'):
		...
		if data_type == '2D':
			...
			theano_args['X'].tag.test_value = np.random.rand(20,1,256,256).astype('float32')
			...
			prediction_train_smax_flat = get_output(self.net[self.cf.out_layer_flat], train_args['X'], deterministic=False)
			prediction_val_smax_flat = get_output(self.net[self.cf.out_layer_flat], val_args['X'], deterministic=True)

			# reshape softmax prediction: shapes (pixels,c) -> (b,c,0,1)
			prediction_train_smax = prediction_train_smax_flat.reshape((train_args['X'].shape[0], self.cf.out_dim[0], self.cf.out_dim[1], self.cf.num_classes)).transpose((0,3,1,2))
			prediction_val_smax = prediction_val_smax_flat.reshape((val_args['X'].shape[0], self.cf.out_dim[0], self.cf.out_dim[1], self.cf.num_classes)).transpose((0,3,1,2))
			...
			prediction_train_smax_flat = get_output(self.net[self.cf.out_layer_flat], train_args['X'],
													deterministic=False)
			prediction_val_smax_flat = get_output(self.net[self.cf.out_layer_flat], val_args['X'], deterministic=True)
		...
		### training functions
		params = get_all_params(self.net[self.cf.out_layer_flat], trainable=True)
		grads = theano.grad(training_loss, params)
		updates = adam(grads, params, learning_rate=train_args['lr'])

		self.train_fn = theano.function(train_args.values(), [self.loss['train'], train_dices_hard], updates=updates)
		self.val_fn = theano.function(val_args.values(), [self.loss['val'], val_dices_hard])
	...
	def run_epoch(self, batch_gen):
		...
		for _ in range(self.cf.num_val_batches):
			...
			if self.cf.loss == 'weighted_cross_entropy':
				...
				args = (batch['data'], batch['seg'], weights)

			val_loss, val_dices = self.val_fn(*args)
		...
		for _ in range(self.cf.num_train_batches):
			...
			args = (batch['data'], batch['seg'], lr)
			if self.cf.loss == 'weighted_cross_entropy':
				weights = get_weights(batch['seg'], self.cf.num_classes)
				args = (batch['data'], batch['seg'], lr, weights )

			train_loss, train_dices = self.train_fn(*args)
		...
		self.metrics['train']['dices'] = np.append(self.metrics['train']['dices'], train_dices_running_batch_mean, axis=0)
	...
	def save_weights(self, outfile_pkl=None, spec=''):
		...
		params = lasagne.layers.get_all_param_values(self.net[self.cf.out_layer_flat])
	...
	def load_weights(self, outfile_pkl=None, spec=''):
		...
		lasagne.layers.set_all_param_values(self.net[self.cf.out_layer_flat], pretrained_weights)
	...
	def plot_architecture(self, f_name=None):
		...
		layers = get_all_layers(self.net[self.cf.out_layer_flat])
